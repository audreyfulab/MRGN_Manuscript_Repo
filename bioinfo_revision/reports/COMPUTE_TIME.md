...
Generated by [`compute_time.R`](../simulation_results/results_scripts/compute_time.R) on 2026-08-28.
...
- [1. Headline: the deployed configuration](#1-headline-the-deployed-configuration)
- [2. By sample size](#2-by-sample-size)
- [3. Every arm](#3-every-arm)
- [4. What these numbers are, and what they are not](#4-what-these-numbers-are-and-what-they-are-not)

---

## What this report is

How long does each method take to fit **one trio**? This is the table form of **Table 3** of the manuscript, with MR-GGI added.

One thing to hold on to before reading any number here: what is timed is the inference call and nothing else. Confounder selection is excluded for all four methods, because it runs once per group and is shared; MRGN's optional bootstrap is excluded too. So these are times to fit one trio *given* a confounder set, which is the quantity that is comparable across methods -- not end-to-end cost. Section 4 spells out what that does and does not license.

The other thing that matters more than method choice is how many covariates a method was handed, which is what section 3 is for.
...
MR-GGI is the clearest case: 7 ms with no covariates against 229 ms under CS-alpha, a factor of ~35 on the same trios, because its cost is quadratic in the covariate count -- it tests every gene pair, and CS-alpha hands it a median of 98 of them. The other two move far less: MRGN spans 4 ms to 34 ms across its four arms (a factor of 8.3) and MRPC 34 ms to 163 ms across its two (a factor of 4.8).
