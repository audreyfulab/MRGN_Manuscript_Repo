...
- [1. What the two selections are](#1-what-the-two-selections-are)
  - [The pool is the thing to hold in mind](#the-pool-is-the-thing-to-hold-in-mind)
- [2. Metrics](#2-metrics)
- [3. Scorecard](#3-scorecard)
  - [3a. Sample-size view](#3a-sample-size-view)
  - [3b. Effect-size view](#3b-effect-size-view)
- [4. Figure 3 -- selection performance against confounder count](#4-figure-3----selection-performance-against-confounder-count)
  - [4a. n = 670, the published layout](#4a-n--670-the-published-layout)
  - [4b. Sample-size view](#4b-sample-size-view)
  - [4c. Effect-size view](#4c-effect-size-view)
- [5. Figure 4 -- what selection errors cost MRGN](#5-figure-4----what-selection-errors-cost-mrgn)
- [6. Selection confusion matrices](#6-selection-confusion-matrices)
  - [6a. Sample-size view](#6a-sample-size-view)
  - [6b. Effect-size view](#6b-effect-size-view)
- [7. FIGURES FROM A SUPERSEDED RUN: METHODS.md Table 17](#7-figures-from-a-superseded-run-methodsmd-table-17)
- [8. Summary](#8-summary)
...
Before any method can work out how a trio is wired it has to decide **which** covariates to adjust for, chosen from a pool of several thousand candidates. That choice is a separate problem from the inference that follows, and this report scores it on its own terms: how much of what a rule selects is genuinely confounding that trio (precision), and how much of that trio's real confounding it finds (recall).

Sections 1-3 define the two rules and score them. Section 4 is the table form of **Figure 3** of the manuscript. Section 5 turns to the question that actually matters -- what selection mistakes cost MRGN downstream -- and is the table form of **Figure 4**. Sections 6-8 give the underlying counts and a summary.

The short version: the two rules fail in opposite directions by design, and it is the confounders a rule **misses** that damage inference, not the ones it wrongly includes.
...
Reproduces `Manuscript/figures/SF7_Confounder_Selection_Impact_On_Inference.pdf`. Precision and recall are set-level quantities and cannot be computed per trio, so the x-axis is grouped and both metrics are computed within each group, then fitted with a **trio-count-weighted LOESS and a 95% confidence ribbon**, matching the published figure. Where the count takes few enough distinct values -- CS-q's false positives run 0-7 -- the grouping is by exact value rather than by quantile bin, since quantile breaks cannot split a variable that takes three values. Points are the group estimates, sized by how many trios back them; a panel with fewer than 5 distinct x values shows those points with no curve, because there is nothing to fit through them.

**Scored over all three trio edges, at the skeleton level.** Each trio contributes whichever of `V1`–`T1`, `V1`–`T2` and `T1`–`T2` its topology actually contains, and an edge is a true positive when the pair is adjacent in both the generating and the inferred topology. Precision is then *edges called that are truly there / edges called*, and recall *edges called that are truly there / edges truly there*, summed over the trios in a bin -- so a bin of 30 trios carries roughly 60 edges rather than 30 binary calls, and a partially-right call scores partially. This is a **wider** question than the `T1`–`T2` row of Table 2, which scores that one edge because it is the only one GMAC and MR-GGI resolve; Figure 4 is MRGN-only and has no such constraint. **Orientation is not scored here**: `M1.1` (`V1 → T1 → T2`) and `M2.1` (`V1 → T1 ← T2`) have the same skeleton, so a mediation trio called a v-structure is a full 2/2. Orientation error is what the model rows of Table 2 measure, and the two should be read together.

**A precision curve that stops short of the panel is a result, not a gap.** Precision is 0/0 in any bin where MRGN named no edges at all -- every trio came back `Other`, whose skeleton is empty -- and that is undefined, not zero, so the curve ends at the last bin that had a value. Recall carries on, because its denominator is the edges the generating topologies contain and is never zero: every model has at least the `V1`–`T1` or `V1`–`T2` edge. **Ticks along the top of a panel mark those bins.** In the CS-α false-negative panel this is the substantive finding: past roughly 30 missed confounders at the small effect treatment, MRGN returns `Other` for every trio in the bin and stops naming edges at all -- 61 trios across the last two bins. Read the ticks as *the method gave no answer here*, not as a precision that fell to zero.

This region is **narrower than it was under the old `T1`–`T2`-only scoring**, which put it at 155 trios across the last five bins. That is the change working as intended, not a shift in the underlying results: scoring one edge made precision undefined whenever no trio was called edge-*present*, which `M0` and `M3` calls satisfy too. Scoring the skeleton, an `M0` call still names `V1`–`T1`, so only a bin of pure `Other` leaves precision with an empty denominator.
...
**Read these with the pool width in view.** The true-negative cell dominates by two orders of magnitude -- roughly 8,000 of every 8,100 pairs are correctly-rejected nulls -- so specificity is above 99.9% for CS-q and just under 99% for CS-alpha in every group, and separates them not at all. That is a property of a 0.3% signal density, not a finding about either method. Precision and recall are the informative margins, which is why §3 leads with them.
